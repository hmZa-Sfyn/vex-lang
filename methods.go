...
func stringMethod(v *Value, key string) *Value {
	...
	case "split":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			sep := ""
			if len(args) > 0 { sep = args[0].Str }
			parts := strings.Split(s, sep)
			arr := make([]*Value, len(parts))
			for i, p := range parts { arr[i] = StringVal(p) }
	...
	case "replace":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			if len(args) < 2 { return StringVal(s), nil }
			return StringVal(strings.ReplaceAll(s, args[0].Str, args[1].Str)), nil
		}}
	case "index":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			if len(args) == 0 { return NumberVal(-1), nil }
	...
	case "slice":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			if len(args) > 0 { start = int(args[0].Num) }
			if len(args) > 1 { end = int(args[1].Num) }
			if start < 0 { start = 0 }
	...
	case "bytes":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			for i, b := range bs { arr[i] = NumberVal(float64(b)) }
			return ArrayVal(arr), nil
		}}
	}
	return NullVal()
...
func arrayMethod(v *Value, key string) *Value {
	...
	case "pop":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			if len(v.Array) == 0 { return NullVal(), nil }
	...
	case "join":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			for i, el := range arr { parts[i] = el.String() }
			return StringVal(strings.Join(parts, sep)), nil
		}}
	case "map":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			if len(args) == 0 { return v, nil }
			fn := args[0]
	...
	case "filter":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			for i, el := range arr {
				r, err := callValue(fn, []*Value{el, NumberVal(float64(i))}, ci)
				if err != nil { return nil, err }
				if r.IsTruthy() { result = append(result, el) }
			}
			return ArrayVal(result), nil
		}}
	case "reduce":
	...
	case "includes":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			if len(args) == 0 { return BoolVal(false), nil }
			for _, el := range arr {
				if el.Equals(args[0]) { return BoolVal(true), nil }
			}
	...
	case "flat":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			var flat []*Value
			for _, el := range arr {
				if el.Type == VAL_ARRAY { flat = append(flat, el.Array...) } else { flat = append(flat, el) }
			}
	...
	case "reverse":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			for i, el := range arr { cp[len(arr)-1-i] = el }
			return ArrayVal(cp), nil
		}}
	case "first":
		if len(arr) == 0 { return NullVal() }
		return arr[0]
	case "last":
		if len(arr) == 0 { return NullVal() }
...
func responseMethod(v *Value, key string) *Value {
	...
	case "json":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			if err != nil { return ErrVal(fmt.Sprintf("json parse error: %s", err.Error())), nil }
			return val, nil
		}}
	case "bytes":
		return &Value{Type: VAL_BUILTIN, Builtin: func(args []*Value, ci *CallInfo) (*Value, error) {
			...
			for i, by := range b { arr[i] = NumberVal(float64(by)) }
			return ArrayVal(arr), nil
		}}
	}
	return NullVal()
...
func callValue(fn *Value, args []*Value, ci *CallInfo) (*Value, error) {
	...
	case VAL_FUNCTION:
		...
		if res.Err != nil { return nil, res.Err }
		if res.Value != nil { return res.Value, nil }
		return NullVal(), nil
	}
